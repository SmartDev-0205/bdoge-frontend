...
function App() {
  ...
  return (
    <div className=" text-center bg-[url('background.jpg')]  content-center flex flex-col bg-no-repeat bg-cover h-screen w-screen overflow-x-hidden overflow-y-auto" >
      <div className="flex flex-row w-screen justify-between sticky top-0 left-0 px-4 sm:px-12 py-2 backdrop-blur-md">
        ...
        <button className='rounded-md bg-yellow-200 text-black text-m px-3 py-1 sm:px-5 sm:py-2 font-bold' onClick={connectWallet}>
          {isConnected ?
      ...
      <div>
        <p className='text-yellow-400 text-md sm:text-xl font-bold mt-32 sm:mt-1 '>
      ...
      <div className='rounded-xl border-2 mx-5 sm:w-1/2 md:w-2/5 self-center py-8 px-10 flex flex-col gap-y-6 my-24 sm:my-48 gradient-box border-yellow-200'>
        <p className='text-xl sm:text-3xl font-black bg-gradient-to-r from-blue-300 to-yellow-300 bg-clip-text text-transparent'>
        ...
        <div className='flex gap-5 flex-col sm:flex-row w-full'>
          <input value={apv_token} onChange={(e) => setApvToken(e.target.value)} className='bg-slate-800 w-full sm:w-1/2 py-2 rounded-md text-center text-white appearance-none focus:border-slate-800 outline-none px-5'></input>
          <button className={`${isConnected ? 'cursor-pointer' : 'cursor-not-allowed'} bg-slate-800 w-full sm:w-1/2 text-gray-400 rounded-md py-2 px-5 self-center font-bold hover:drop-shadow-2xl`} onClick={claimAirdrop} disabled={!isConnected}>Claim Airdrop</button>
        </div>
      </div>
    </div >
